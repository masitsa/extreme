...
class Payroll extends accounts 
{
	...
	*/
	...
	public function print_payroll_pdf($payroll_id)
	{
		$where = 'personnel_status = 1';
		...
		$date = date('F_Y',strtotime('01-'.$month_id.'-'.$payroll_year));
		$this->load->library('pdf');
		$this->pdf->load_view('payroll/payroll', $data);
		$this->pdf->set_paper("A4", "landscape");
		$this->pdf->render();
		$this->pdf->stream("Payroll_for_".$date);
	}

	public function export_payroll($payroll_id)
	{
		$where = 'personnel_status = 1';
		...
		$date = date('F_Y',strtotime('01-'.$month_id.'-'.$payroll_year));
		$data['date'] = "Payroll_for_".$date;
		
		$this->payroll_model->export_payroll($data);
	}
	...
	*
	*/
	public function payroll_payslips($payroll_id, $order = 'personnel_onames', $order_method = 'ASC') 
	{
		$branch_id = $this->session->userdata('branch_id');
		$branch_name = $this->session->userdata('branch_name');
		$branches = $this->branches_model->all_branches();
		...
		$segment = 5;
		$this->load->library('pagination');
		$config['base_url'] = site_url().'accounts/payroll-payslips/'.$payroll_id.'/'.$order.'/'.$order_method;
		$config['total_rows'] = $this->users_model->count_items($table, $where);
		$config['uri_segment'] = $segment;
		$config['per_page'] = 20;
		$config['num_links'] = 5;
		...
		$data['title'] = $v_data['title'] = $branch_name.' payslips';
		...
		$data['content'] = $this->load->view('payroll/payroll_payslips', $v_data, true);
		
		$this->load->view('admin/templates/general_page', $data);
	}

	public function print_payslip($personnel_id, $payroll_id)
	{
		$where = 'payroll.branch_id = branch.branch_id AND payroll.payroll_id = '.$payroll_id;
		$this->db->where($where);
		$branches = $this->db->get('payroll, branch');
	...
	public function download_payslip($personnel_id, $payroll_id)
	{
		$where = 'payroll.branch_id = branch.branch_id AND payroll.payroll_id = '.$payroll_id;
		$this->db->where($where);
		$branches = $this->db->get('payroll, branch');
		...
		$this->load->library('pdf');
		$this->pdf->load_view('payroll/payslip_pdf', $data);
		$this->pdf->set_base_path(base_url()."assets/themes/porto-admin/1.4.1/");
		//$this->pdf->set_paper("A4", "portrait");
		$paper_size = array(0,0,400,1000);
		$this->pdf->set_paper($paper_size);
		$this->pdf->render();
		$this->pdf->stream("Payslip_for_".$fname."_".$onames.'_'.$date);
	}
}
?>