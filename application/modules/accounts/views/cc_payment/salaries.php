<?php
		...
		if ($query->num_rows() > 0)
		{
			...
			foreach ($query->result() as $row)
			{
				...
				//get salary details
				$payments = $this->cc_payment_model->payments_view($personnel_id);
				$benefits = $this->cc_payment_model->benefits_view($personnel_id);
				$allowances = $this->cc_payment_model->allowances_view($personnel_id);
				$deductions = $this->cc_payment_model->deductions_view($personnel_id);
				$other_deductions = $this->cc_payment_model->other_deductions_view($personnel_id);
				$savings = $this->cc_payment_model->savings_view($personnel_id);
				$loan_schemes = $this->cc_payment_model->scheme_view($personnel_id);
				...
				$nssf = $this->cc_payment_model->nssf_view($gross);
				$paye = $this->cc_payment_model->calculate_paye($taxable - $nssf);
				$paye = $paye - ($insurance_relief + $monthly_relief);
				$nhif = $this->cc_payment_model->nhif_view($gross);
				...
				'
					<tr>
						...
						<td><a href="'.site_url().'accounts/cc-payment/view-payslip/'.$personnel_id.'" class="btn btn-sm btn-info" title="Payslip for '.$personnel_name.'" target="_blank">Payslip</td>
					</tr> 
				';
			}
...
?>
 ...
 <section class="panel">
   ...
   <div class="panel-body">
    	<div class="padd">
			<?php
            echo form_open("accounts/cc_payment_model/search_personnel", array("class" => "form-horizontal"));
            ?>
            <div class="row">
                <div class="col-md-6">
						...
						<section class="panel">
							...
							<div class="panel-body">
								...
								if(!empty($search))
								{
									...
									echo '<a href="'.site_url().'accounts/cc-payment/close_search" class="btn btn-sm btn-info pull-left">Close search</a>';
								}
                            	...
                            	<div class="row" style="margin-bottom:20px;">
                                    <div class="col-sm-2 col-sm-offset-10">
                                        <a href="<?php echo site_url();?>accounts/cc-payment" class="btn btn-sm btn-warning pull-right">Back to cc_payment</a>
