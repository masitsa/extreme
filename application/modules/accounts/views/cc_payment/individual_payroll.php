	...
	$total_net = 0;
		...
		if ($query->num_rows() > 0)
		{
			...
			foreach ($query->result() as $row)
			{
				...
				$tables = $this->db->get('cc_payment, branch');
				...
				$cc_payment = $this->cc_payment_model->get_cc_payment($cc_payment_id);
				$individual_query = $this->personnel_model->retrieve_cc_payment_personnel($where);	
				if ($individual_query->num_rows() > 0)
				{
					...
					$total_net = 0;
						...
						*/
						...
						$nssf = $this->cc_payment_model->get_cc_payment_amount($personnel_id, $cc_payment_id, $table, 1);
						$total_nssf += $nssf;
						...
						if($deductions->num_rows() > 0)
						{
							foreach($deductions->result() as $res)
							{
								...
								$deduction_amt = $this->cc_payment_model->get_cc_payment_amount($personnel_id, $cc_payment_id, $table, $table_id);
						...
						if($other_deductions->num_rows() > 0)
						{
							foreach($other_deductions->result() as $res)
							{
								...
								$other_deduction_amt = $this->cc_payment_model->get_cc_payment_amount($personnel_id, $cc_payment_id, $table, $table_id);
						...
						if($rs_savings->num_rows() > 0)
						{
							foreach($rs_savings->result() as $res)
							{
								...
								//get schemes
								$total_savings += $this->cc_payment_model->get_cc_payment_amount($personnel_id, $cc_payment_id, $table, $savings_id);
							}
						}
						...
						if($rs_schemes->num_rows() > 0)
						{
							foreach($rs_schemes->result() as $res)
							{
								...
								//get schemes
								$total_schemes += $this->cc_payment_model->get_cc_payment_amount($personnel_id, $cc_payment_id, $table, $loan_scheme_id);
						...
						'
							<tr>
								...
								<td><a href="'.site_url().'print-payslip/'.$cc_payment_id.'" class="btn btn-sm btn-primary" title="Print '.$cc_payment_name.'" target="_blank">Payslip</a></td>
							</tr> 
						';
