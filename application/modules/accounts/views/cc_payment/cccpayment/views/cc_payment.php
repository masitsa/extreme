...
if ($query->num_rows() > 0)
{
	...
	foreach ($query->result() as $row)
	{
		...
		*/
		...
		$nhif = $this->cc_payment_model->get_cc_paymentamount($personnel_id, $cc_payment_id, $table, 1);
		$total_nhif += $nhif;
		...
		if($other_deductions->num_rows() > 0)
		{
			foreach($other_deductions->result() as $res)
			{
				...
				$other_deduction_amt = $this->cc_paymentl_model->get_cc_payment_amount($personnel_id, $cc_payment_id, $table, $table_id);
