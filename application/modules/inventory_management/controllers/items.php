...
class items extends MX_Controller  
{
	...
	*/
	...
	{
		$this->form_validation->set_rules('asset_barcode', 'Barcode', 'required|xss_clean');
		if ($this->form_validation->run())
		{
			$item_id = $this->items_model->add_inventory_item($item_id);
			$this->session->set_userdata('success_message', 'inventory updated successfully');
			redirect('inventory/edit-item/'.$item_id);
		}
		
		else
		{
			$this->session->set_userdata('error_message', 'inventory not updated');
			$this->add_inventory_item($item_id);
		}
	}
	...
	*/
	public function add_item($item_id = NULL) 
	{
		...
		$v_data['all_models'] = $this->items_model->get_all_models();
		$v_data['all_brands'] = $this->items_model->get_all_brands();
		$v_data['all_manufacturers'] = $this->items_model->get_all_manufacturers();
		$v_data['all_categories'] = $this->items_categories_model->all_categories();
		$v_data['all_personnel'] = $this->personnel_model->all_personnel();
		$v_data['all_locations'] = $this->items_model->get_all_locations();
		$v_data['all_status'] = $this->items_model->get_all_status();
		$v_data['all_conditions'] = $this->items_model->all_conditions();
	...
	*/
	public function edit_item($item_id) 
	{
		...
		if ($query->num_rows() > 0)
		{
			...
			$v_data['all_status'] = $this->items_model->get_all_status();
			$v_data['all_conditions'] = $this->items_model->all_conditions();
			$v_data['inventory_details'] =$this->items_model->get_item_inventory($item_id);
	...
	public function edit_inventory_item($inventory_id)
	{
		if($this->items_model->update_inventory_item($inventory_id))
			{
				$this->session->set_userdata('success_message', 'Updated Successfully');
				redirect('inventory/item');
			}
			...
			{
				$this->session->set_userdata('error_message', 'Could not update the inventory. Please try again');
				redirect('inventory/item');
			}
	}
	...
	{
		if($this->items_model->delete_inventory_item($inventory_id))
			{
				$this->session->set_userdata('success_message', 'Deleted Successfully');
				redirect('inventory/item');
			}
			
			else
			{
				$this->session->set_userdata('error_message', 'Could not delete. Please try again');
				redirect('inventory/item');
			}
	}
	//search
