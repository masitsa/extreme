...
class Ultrasound_model extends CI_Model 
{
	...
	function save_tests_format2($visit_id)
	{
		...
		$data['ultrasound_visit_results_result'] = $result;
		$this->db->where(array('ultrasound_visit_result_format' => $format, 'visit_id' => $visit_id));
		
		$this->db->update('ultrasound_visit_results', $data);
	}
